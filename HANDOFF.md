...
## 2026-09-04 — "How It Was Built" page

Added `public/how-it-was-built/index.html` plus twelve resized screenshots under `public/how-it-was-built/img/`. It is a plain-language, timestamped play-by-play of the five Codex sessions above, written for non-technical readers and retold from the raw session logs in `~/.codex/sessions/2026/09/04/` (23 threads, about 80 MB). Vite copies `public/` into `dist/`, so the page ships with the game at `/how-it-was-built/` on Pages; README links to it. It is static, self-contained, uses the portfolio's default font stack, and has no dependency on the game code. Uncommitted at time of writing. If game facts change (test counts, controls), the page describes the evening of 2026-09-04 and should not be updated to match; add a new entry instead.
