...
## Final pre-human-test stadium pass

Revisited the same local official promotional capture, focusing on its recessed goal architecture, repeating metal supports and visible floodlight arrays. Compared the running baseline and revised goal from the same camera position `(25, 5, -80)`, looking at `(0, 8, -105)`. Added a tunnel shell with ribs, panel texture and illuminated edges, plus end-bowl floodlight banks. Reduced cage opacity and removed its face across the goal mouth. Batched static opaque architecture without changing the physics colliders. This is a qualitative reference-led refinement; lighting, stands, turf and the procedural car remain visibly simpler than the original.
