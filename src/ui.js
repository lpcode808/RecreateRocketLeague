export class UI {
  constructor(onAction){
    ...
    this.el['controls-details'].insertAdjacentHTML('beforebegin',`<div class="touch-help"><p><strong>Left thumb:</strong> drag up to drive, down to reverse, sideways to steer. In the air, the stick controls pitch and yaw.</p><p><strong>Right thumb:</strong> hold BOOST or JUMP for height. Release and tap JUMP again to flip; aim with the stick. DRIFT helps with tight turns. ROLL tilts you in the air.</p><button id="recover-touch">RECOVER CAR</button></div>`);
    document.querySelector('#recover-touch').addEventListener('click',()=>onAction('reset'));
